...
| 数据库凭据 | user=`tokenpanel`, password=`tokenpanel`, dbname=`tokenpanel` |
...
psql -U tokenpanel -h 127.0.0.1 -d tokenpanel -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER tokenpanel WITH PASSWORD 'tokenpanel';
...
```bash
# 连接数据库
psql -U tokenpanel -h 127.0.0.1 -d tokenpanel
...
# 执行 SQL 文件
psql -U tokenpanel -h 127.0.0.1 -d tokenpanel -f migration.sql
```
