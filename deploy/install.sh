#!/bin/bash
#
# TokenPanel Installation Script
# TokenPanel 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/imleoo/tokenpanel/zhiguofan/deploy/install.sh | sudo bash
#
...
# Configuration
GITHUB_REPO="imleoo/tokenpanel"
INSTALL_DIR="/opt/tokenpanel"
SERVICE_NAME="tokenpanel"
SERVICE_USER="tokenpanel"
CONFIG_DIR="/etc/tokenpanel"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="TokenPanel 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 TokenPanel 服务："
    ...
    ["not_installed"]="TokenPanel 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 TokenPanel。"
    ...
    ["uninstall_complete"]="TokenPanel 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 TokenPanel"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 TokenPanel"
    ...
    ["server_config_desc"]="配置 TokenPanel 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="TokenPanel Installation Script"
    ...
    # Completion
    ["install_complete"]="TokenPanel installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start TokenPanel service:"
    ...
    # Upgrade
    ["upgrading"]="Upgrading TokenPanel..."
    ...
    ["not_installed"]="TokenPanel is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove TokenPanel from your system."
    ...
    ["uninstall_complete"]="TokenPanel has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install TokenPanel"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove TokenPanel"
    ...
    ["server_config_desc"]="Configure TokenPanel server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/tokenpanel" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/tokenpanel" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="tokenpanel_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/tokenpanel" "$INSTALL_DIR/tokenpanel"
    chmod +x "$INSTALL_DIR/tokenpanel"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/tokenpanel"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/tokenpanel.service << EOF
[Unit]
Description=TokenPanel - AI API Gateway Platform
Documentation=https://github.com/imleoo/tokenpanel
...
User=tokenpanel
Group=tokenpanel
WorkingDirectory=/opt/tokenpanel
ExecStart=/opt/tokenpanel/tokenpanel
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=tokenpanel
...
start_service() {
    ...
    if systemctl start tokenpanel; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u tokenpanel -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable tokenpanel 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status tokenpanel"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u tokenpanel -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart tokenpanel"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop tokenpanel"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if TokenPanel is installed
    if [ ! -f "$INSTALL_DIR/tokenpanel" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/tokenpanel" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet tokenpanel; then
        print_info "$(msg 'stopping_service')"
        systemctl stop tokenpanel
    fi

    # Backup current binary
    cp "$INSTALL_DIR/tokenpanel" "$INSTALL_DIR/tokenpanel.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/tokenpanel.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/tokenpanel"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start tokenpanel

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: TokenPanel must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/tokenpanel" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="tokenpanel.backup.${current_version}"
        else
            backup_name="tokenpanel.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/tokenpanel" "$INSTALL_DIR/$backup_name"
...
main() {
    ...
    case "${1:-}" in
        ...
        --help|-h)
            ...
            echo "  $0 upgrade -v v1.0.1      # Upgrade to specific version"
