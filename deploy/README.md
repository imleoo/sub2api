# TokenPanel Deployment Files

This directory contains files for deploying TokenPanel on Linux servers.
...
| `tokenpanel.service` | Systemd service unit file |
| `tokenpanel-datamanagementd.service` | datamanagementd systemd service unit file |
...
curl -sSL https://raw.githubusercontent.com/Wei-Shaw/tokenpanel/main/deploy/docker-deploy.sh -o docker-deploy.sh
...
3. If `ADMIN_PASSWORD` is not set, check logs for the generated password:
   ```bash
   docker compose logs tokenpanel | grep "admin password"
   ```
...
# Restart TokenPanel only
docker compose -f docker-compose.local.yml restart tokenpanel
...
# Restart TokenPanel only
docker compose restart tokenpanel
...
| `ADMIN_EMAIL` | No | `admin@tokenpanel.local` | Admin email |
...
docker compose -f docker-compose.local.yml down
cd ..
tar czf tokenpanel-complete.tar.gz deployment/

# Transfer to new server
scp tokenpanel-complete.tar.gz user@new-server:/path/to/destination/
...
TokenPanel supports three methods to connect to Gemini:
...
2. Extract and copy the binary to `/opt/tokenpanel/`
3. Copy `tokenpanel.service` to `/etc/systemd/system/`
4. Run:
   ```bash
...
# Stop the service
sudo systemctl stop tokenpanel
...
1. Edit the systemd service:
   ```bash
   sudo systemctl edit tokenpanel
   ```
...
1. Edit the service file:
   ```bash
   sudo nano /etc/systemd/system/tokenpanel.service
   ```
...
The main config file is at `/etc/tokenpanel/config.yaml` (created by Setup Wizard).
...
```
/opt/tokenpanel/
├── tokenpanel              # Main binary
├── tokenpanel.backup       # Backup (after upgrade)
└── data/                # Runtime data
...
# View detailed logs
docker compose -f docker-compose.local.yml logs --tail=100 tokenpanel
...
# View detailed logs
docker compose logs --tail=100 tokenpanel
...
# View recent logs
sudo journalctl -u tokenpanel -n 50

# Check config file
sudo cat /etc/tokenpanel/config.yaml
...
TokenPanel supports TLS fingerprint simulation to make requests appear as if they come from the official Claude CLI (Node.js client).

> **💡 Tip:** Visit **[tls.tokenpanel.org](https://tls.tokenpanel.org/)** to get TLS fingerprint information for different devices and browsers.
