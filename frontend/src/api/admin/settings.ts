...
// ── 平台限额类型 ──────────────────────────────────────────────────
export type PlatformType = "anthropic" | "openai" | "gemini"
...
const PLATFORMS: PlatformType[] = ["anthropic", "openai", "gemini"]

/** 归一化为全 3 平台 × 3 窗口（缺失填 null），供模板非空绑定 */
...
/** 提交前清洗：非有限数/负数/空字符串 → null（保留 0 = 显式禁用），返回全 3 平台嵌套 map */
