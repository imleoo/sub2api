export default {
  ...
  admin: {
    ...
    dashboard: {
      ...
      modelDistribution: '模型分布',
      groupDistribution: '分组使用分布',
      // Phase 1 P1-4：Provider / Account 跨 group 聚合视图
    ...
    accounts: {
      ...
      generic: {
        ...
        syncModels: '同步最新模型',
        syncModelsLoading: '同步中…',
        syncModelsApiKey: '同步用 API Key',
        syncModelsApiKeyPlaceholder: '仅用于拉取模型列表，不会随账号保存',
        syncModelsHint: '使用上方 Base URL + 此 API Key 请求 /v1/models，拉取到的模型写入模型定价表，可在「模型定价」中设置折扣。',
        syncModelsSuccess: '已拉取 {count} 个模型并写入模型定价表',
        syncModelsFailed: '同步模型失败',
        syncModelsNeedBaseUrl: '请先填写上游地址（Base URL）',
        syncModelsNeedApiKey: '请先填写同步用 API Key',
      },
      types: {
        oauth: 'OAuth',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个 tokenpanel 实例的场景。',
  ...
  onboarding: {
    ...
    user: {
      welcome: {
        title: '👋 欢迎使用 TokenPanel',
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">您好！欢迎来到 TokenPanel AI 服务平台。</p><p style="margin-bottom: 12px;"><b>🎯 快速开始：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>🔑 创建 API 密钥</li><li>📋 复制密钥到您的应用</li><li>🚀 开始使用 AI 服务</li></ul><p style="color: #10b981; font-weight: 600;">只需 1 分钟，让我们开始吧 →</p></div>',
        nextBtn: '开始 🚀',
        prevBtn: '跳过'
      },
      keyManage: {
