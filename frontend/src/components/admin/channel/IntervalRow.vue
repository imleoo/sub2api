<template>
  <div class="flex items-start gap-2 rounded border p-2"
    ...
    <!-- Token mode: context range + prices (currency/MTok) -->
    <template v-if="mode === 'token'">
      ...
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.inputPrice', '输入') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">{{ currencyLabel() }}/M</span></label>
        <input :value="displayCurrencyInput(interval.input_price)" @input="emitCurrencyField('input_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.outputPrice', '输出') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">{{ currencyLabel() }}/M</span></label>
        <input :value="displayCurrencyInput(interval.output_price)" @input="emitCurrencyField('output_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheWritePrice', '缓存W') }} <span class="text-gray-300">{{ currencyLabel() }}/M</span></label>
        <input :value="displayCurrencyInput(interval.cache_write_price)" @input="emitCurrencyField('cache_write_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.cacheReadPrice', '缓存R') }} <span class="text-gray-300">{{ currencyLabel() }}/M</span></label>
        <input :value="displayCurrencyInput(interval.cache_read_price)" @input="emitCurrencyField('cache_read_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
    </template>
    ...
    <template v-else>
      ...
      <div class="flex-1">
        <label class="text-xs text-gray-400">{{ t('admin.channels.form.perRequestPrice', '单次价格') }} <span v-if="isEmpty" class="text-red-500">*</span> <span class="text-gray-300">{{ currencyLabel() }}</span></label>
        <input :value="displayCurrencyInput(interval.per_request_price)" @input="emitCurrencyField('per_request_price', ($event.target as HTMLInputElement).value)"
          type="number" step="any" min="0" class="input mt-0.5 text-xs" />
      </div>
    </template>
...
function displayCurrencyInput(value: number | string | null | undefined) {
  if (value === null || value === undefined || value === '') return ''
  const parsed = Number(value)
  return Number.isFinite(parsed) ? toDisplayCurrencyAmount(parsed) : ''
}

function emitCurrencyField(field: keyof IntervalFormEntry, value: string) {
  if (value === '') {
    emit('update', { ...props.interval, [field]: null })
    return
  }
  const parsed = Number(value)
  emit('update', {
    ...props.interval,
    [field]: Number.isFinite(parsed) ? fromDisplayCurrencyAmount(parsed) : null,
  })
}
