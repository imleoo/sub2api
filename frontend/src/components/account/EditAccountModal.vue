<template>
  ...
  >
    ...
    >
      ...
      <!-- Header Override Section (anthropic/openai apikey + grok apikey) -->
      ...
      <!-- Anthropic: Response Masking (Kiro Compat) -->
      <div
        v-if="account?.platform === 'anthropic'"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.anthropic.responseMasking') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.anthropic.responseMaskingDesc') }}
            </p>
          </div>
          <button
            type="button"
            @click="responseMaskingEnabled = !responseMaskingEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              responseMaskingEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                responseMaskingEnabled ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
...
const baseUrlHint = computed(() => {
  ...
  if (props.account.platform === 'lingjing') return '留空即可，后端已内置京东云灵境 API 地址'
...
const openaiResponsesWebSocketV2Mode = computed({
  get: () => openaiAPIKeyResponsesWebSocketV2Mode.value,
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  // Load OpenAI passthrough toggle (OpenAI API Key)
  ...
  // Load header override state (anthropic/openai apikey + grok apikey)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.type === 'apikey' && props.account.platform !== 'generic') {
      ...
      // For account types without credential editing here, only update intercept_warmup_requests if changed
      const currentCredentials = (props.account.credentials as Record<string, unknown>) || {}
      const newCredentials: Record<string, unknown> = { ...currentCredentials }
    ...
    // Response masking applies to all Anthropic accounts
    if (props.account.platform === 'anthropic') {
      const currentExtra = (updatePayload.extra as Record<string, unknown>) || (props.account.extra as Record<string, unknown>) || {}
      const newExtra: Record<string, unknown> = { ...currentExtra }
      if (responseMaskingEnabled.value) {
    ...
    // For OpenAI API Key accounts, handle passthrough mode in extra
