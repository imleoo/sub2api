...
const platformCards = computed<FusedPlatformCard[]>(() => {
  ...
  const PLATFORM_ORDER = ['anthropic', 'openai', 'gemini']
...
const formatBalance = (b: number) => {
  if (appStore.currencyMode === 'cny') return `¥${(b * appStore.cnyRate).toFixed(2)}`
  return `$${new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(b)}`
}

const formatNumber = (n: number) => n.toLocaleString()
const formatCost = (c: number) => {
  const v = appStore.currencyMode === 'cny' ? c * appStore.cnyRate : c
