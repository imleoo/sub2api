...
import { currencyLabel, formatReasoningEffort, formatUSD } from '@/utils/format'
...
const exportToExcel = async () => {
  ...
  try {
    ...
    const headers = [
      ...
      `${t('admin.usage.inputCost')} (${currencyLabel()})`, `${t('admin.usage.outputCost')} (${currencyLabel()})`,
      `${t('admin.usage.cacheReadCost')} (${currencyLabel()})`, `${t('admin.usage.cacheCreationCost')} (${currencyLabel()})`,
      t('usage.rate'), t('usage.accountMultiplier'), `${t('usage.original')} (${currencyLabel()})`, `${t('usage.userBilled')} (${currencyLabel()})`, `${t('usage.accountBilled')} (${currencyLabel()})`,
    ...
    while (true) {
      ...
      const rows = (res.items || []).map((log: AdminUsageLog) => [
        ...
        formatUSD(log.input_cost, 6), formatUSD(log.output_cost, 6),
        formatUSD(log.cache_read_cost, 6), formatUSD(log.cache_creation_cost, 6),
        log.rate_multiplier?.toPrecision(4) || '1.00', (log.account_rate_multiplier ?? 1).toPrecision(4),
        formatUSD(log.total_cost, 6), formatUSD(log.actual_cost, 6),
        formatUSD((log.account_stats_cost ?? log.total_cost ?? 0) * (log.account_rate_multiplier ?? 1), 6), log.first_token_ms ?? '', log.duration_ms,
