...
折扣配置文件：`backend/data/model_discounts.json`（服务器本地文件，修改后重启生效）
...
- 折扣率范围：`0~1`，如 `0.8` = 8折（用户实际扣费为原价的 80%）
- 未配置的模型默认按原价计费
- 模型 ID 区分大小写，需与定价文件中的 ID 完全一致

### 7.2 人民币汇率

默认汇率为 `7`（1 USD = 7 CNY），可在配置文件中通过 `pricing.cny_rate` 修改：
