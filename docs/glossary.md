...
| 术语 | 含义 | 取值 |
|---|---|---|
| `platform` | 路由 / 调度 / UI 分类字段（账号、分组通用） | `anthropic` / `openai` / `gemini` / `antigravity`（Phase 5 可能扩 `generic`） |
...
- **总工期**：14 周（不含 Phase 5 观察期） / 22 周（含 Phase 5 的 4 周双桶并存观察）
...
|---|---|---|---|---|
| 0 | 计费基础设施先行 | W1–W2 | 6 | UsageLog 上游成本快照 + `provider_pricing` 表 |
| 1 | 轻量 Generic + provider_key 规范化 | W3–W6 | 5 | `extra.provider` 写入 + 跨 group 聚合视图 |
| 2 | InboundProtocol / OutboundProtocol 双写 | W5–W8 | 5 | `group.inbound_protocol` + `account.outbound_protocol` |
| 3 | Bridge Registry + N×M 矩阵收口 | W9–W12 | 6 | ProtocolBridge 抽象 + 新桥拼装 |
| 4 | Gemini 入站桥 | W13–W14 | 3 | `gemini_v1beta` 入站接入桥矩阵 |
| 5 | Generic 多 Endpoint + Scheduler 重构 | W15–W22 | 6 | `endpoint` 实体 + 双桶并存验证 |
...
`Platform{Anthropic,OpenAI,Gemini,Antigravity}` 常量当前命中：

- 非测试 Go 文件：约 48 个
- 含测试：约 166 个

> 数字会随提交波动，以 `grep -rl 'Platform\(Anthropic\|OpenAI\|Gemini\|Antigravity\)' backend/internal/ backend/cmd/` 实测为准。此处仅用于 Phase 2 改造规模评估。
