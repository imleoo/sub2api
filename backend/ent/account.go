...
type Account struct {
	...
	// 出站协议（Phase 2 引入；空 = 按 platform/type 派生）
...
// was not loaded in eager-loading.
func (e AccountEdges) EndpointsOrErr() ([]*Endpoint, error) {
	if e.loadedTypes[3] {
...
func (*Account) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case account.FieldName, account.FieldNotes, account.FieldPlatform, account.FieldOutboundProtocol, account.FieldType, account.FieldStatus, account.FieldErrorMessage, account.FieldTempUnschedulableReason, account.FieldSessionWindowStatus:
...
// QueryEndpoints queries the "endpoints" edge of the Account entity.
func (_m *Account) QueryEndpoints() *EndpointQuery {
	return NewAccountClient(_m.config).QueryEndpoints(_m)
}
