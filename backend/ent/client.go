...
func (c *Client) Use(hooks ...Hook) {
	for _, n := range []interface{ Use(...Hook) }{
		...
		c.ChannelMonitorRequestTemplate, c.CompositeModelRoute, c.Endpoint,
...
// QueryEndpoints queries the endpoints edge of a Account.
func (c *AccountClient) QueryEndpoints(_m *Account) *EndpointQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, account.EndpointsTable, account.EndpointsColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// UpdateOneID returns an update builder for the given id.
func (c *EndpointClient) UpdateOneID(id int64) *EndpointUpdateOne {
...
// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *EndpointClient) DeleteOneID(id int64) *EndpointDeleteOne {
...
func (c *EndpointClient) Get(ctx context.Context, id int64) (*Endpoint, error) {
	return c.Query().Where(endpoint.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *EndpointClient) GetX(ctx context.Context, id int64) *Endpoint {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryAccount queries the account edge of a Endpoint.
func (c *EndpointClient) QueryAccount(_m *Endpoint) *AccountQuery {
	...
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, endpoint.AccountTable, endpoint.AccountColumn),
		)
		fromV = sqlgraph.Neighbors(_m.driver.Dialect(), step)
		return fromV, nil
	}
...
// LingjingTaskClient is a client for the LingjingTask schema.
type LingjingTaskClient struct {
	config
}

// NewLingjingTaskClient returns a client for the LingjingTask from the given config.
func NewLingjingTaskClient(c config) *LingjingTaskClient {
	return &LingjingTaskClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `lingjingtask.Hooks(f(g(h())))`.
func (c *LingjingTaskClient) Use(hooks ...Hook) {
	c.hooks.LingjingTask = append(c.hooks.LingjingTask, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `lingjingtask.Intercept(f(g(h())))`.
func (c *LingjingTaskClient) Intercept(interceptors ...Interceptor) {
	c.inters.LingjingTask = append(c.inters.LingjingTask, interceptors...)
}

// Create returns a builder for creating a LingjingTask entity.
func (c *LingjingTaskClient) Create() *LingjingTaskCreate {
	mutation := newLingjingTaskMutation(c.config, OpCreate)
	return &LingjingTaskCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of LingjingTask entities.
func (c *LingjingTaskClient) CreateBulk(builders ...*LingjingTaskCreate) *LingjingTaskCreateBulk {
	return &LingjingTaskCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *LingjingTaskClient) MapCreateBulk(slice any, setFunc func(*LingjingTaskCreate, int)) *LingjingTaskCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &LingjingTaskCreateBulk{err: fmt.Errorf("calling to LingjingTaskClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*LingjingTaskCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for LingjingTask.
func (c *LingjingTaskClient) Update() *LingjingTaskUpdate {
	mutation := newLingjingTaskMutation(c.config, OpUpdate)
	return &LingjingTaskUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *LingjingTaskClient) UpdateOne(_m *LingjingTask) *LingjingTaskUpdateOne {
	mutation := newLingjingTaskMutation(c.config, OpUpdateOne, withLingjingTask(_m))
	return &LingjingTaskUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *LingjingTaskClient) UpdateOneID(id int64) *LingjingTaskUpdateOne {
	mutation := newLingjingTaskMutation(c.config, OpUpdateOne, withLingjingTaskID(id))
	return &LingjingTaskUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for LingjingTask.
func (c *LingjingTaskClient) Delete() *LingjingTaskDelete {
	mutation := newLingjingTaskMutation(c.config, OpDelete)
	return &LingjingTaskDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *LingjingTaskClient) DeleteOne(_m *LingjingTask) *LingjingTaskDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *LingjingTaskClient) DeleteOneID(id int64) *LingjingTaskDeleteOne {
	builder := c.Delete().Where(lingjingtask.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &LingjingTaskDeleteOne{builder}
}

// Query returns a query builder for LingjingTask.
func (c *LingjingTaskClient) Query() *LingjingTaskQuery {
	return &LingjingTaskQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeLingjingTask},
		inters: c.Interceptors(),
	}
}

// Get returns a LingjingTask entity by its id.
func (c *LingjingTaskClient) Get(ctx context.Context, id int64) (*LingjingTask, error) {
	return c.Query().Where(lingjingtask.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *LingjingTaskClient) GetX(ctx context.Context, id int64) *LingjingTask {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// Hooks returns the client hooks.
func (c *LingjingTaskClient) Hooks() []Hook {
	return c.hooks.LingjingTask
}

// Interceptors returns the client interceptors.
func (c *LingjingTaskClient) Interceptors() []Interceptor {
	return c.inters.LingjingTask
}

func (c *LingjingTaskClient) mutate(ctx context.Context, m *LingjingTaskMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&LingjingTaskCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&LingjingTaskUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&LingjingTaskUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&LingjingTaskDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown LingjingTask mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		...
		Endpoint, EnterpriseProfile, ErrorPassthroughRule, Group, IdempotencyRecord,
