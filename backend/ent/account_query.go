...
func (_q *AccountQuery) QueryEndpoints() *EndpointQuery {
...
func (_q *AccountQuery) WithEndpoints(opts ...func(*EndpointQuery)) *AccountQuery {
	query := (&EndpointClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := _q.withEndpoints; query != nil {
		if err := _q.loadEndpoints(ctx, query, nodes,
			func(n *Account) { n.Edges.Endpoints = []*Endpoint{} },
			func(n *Account, e *Endpoint) { n.Edges.Endpoints = append(n.Edges.Endpoints, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *AccountQuery) loadEndpoints(ctx context.Context, query *EndpointQuery, nodes []*Account, init func(*Account), assign func(*Account, *Endpoint)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(endpoint.FieldAccountID)
	}
	query.Where(predicate.Endpoint(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.EndpointsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
