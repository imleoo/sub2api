...
func setupAccountMixedChannelRouter(adminSvc *stubAdminService) *gin.Engine {
	...
	router := gin.New()
	accountHandler := NewAccountHandler(adminSvc, nil, nil, nil, nil, nil, nil, nil, nil, nil)
...
func TestAccountHandlerCheckMixedChannelNoRisk(t *testing.T) {
	...
	require.Equal(t, "gemini", adminSvc.lastMixedCheck.platform)
...
func TestAccountHandlerCheckMixedChannelWithRisk(t *testing.T) {
	...
	require.Equal(t, "Gemini", details["current_platform"])
