...
func TestUpdateUserPlatformQuotas_Success(t *testing.T) {
	...
	// 缓存失效：请求中 2 个 platform + 软删除的 1 个 platform（gemini）= 3 次
	if len(cache.deleteCalls) != 3 {
		t.Errorf("expected 3 cache delete calls, got %d: %+v", len(cache.deleteCalls), cache.deleteCalls)
	}
}
...
func TestUpdateUserPlatformQuotas_RejectsTooManyEntries(t *testing.T) {
	h := buildTestHandler(&upsertCapturingQuotaRepo{}, &billingCacheStub{})
	body := `{"quotas":[
		{"platform":"anthropic"},{"platform":"openai"},{"platform":"gemini"},{"platform":"openai"},{"platform":"anthropic"}
	]}`
	c, w := putReq(t, body)
	h.UpdateUserPlatformQuotas(c)
	if w.Code != http.StatusBadRequest {
