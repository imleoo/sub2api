...
// Do 固定返回一个可 failover 的 HTTP 5xx 错误。
// fork 注：原上游版本用 HTTP 200 + SSE 内嵌 "type":"error" 事件模拟失败——这是
// ChatGPT web 后端在 OAuth 逆向订阅（功能 35，已删）场景下的特有行为：
// 官方 xAI/OpenAI apikey 上游对真实错误一律返回标准 HTTP 错误状态码，不会用
// 200 状态码夹带错误事件；apikey 非流式路径也不再做 SSE 内容级错误探测
// （该探测逻辑随 OAuth 专属 handler 一并移除）。因此这里改为返回真实的
// HTTP 502，以贴近 apikey 账号的实际失败信号，驱动 handler 层账号级 failover。
	...
	accounts := []service.Account{
		{
			...
			Concurrency: 0,
			Priority:    0,
			Credentials: map[string]any{"api_key": "token-1"},
		},
		{
			ID:          2,
			...
			Concurrency: 0,
			Priority:    1,
			Credentials: map[string]any{"api_key": "token-2"},
		},
	}
