...
func (s *GroupRepoSuite) TestCreateFromSourcePreservesPriorityAndFiltersIneligibleAccounts() {
	...
	apiKeyID1 := insertAccount("duplicate-apikey-1", service.AccountTypeAPIKey, false)
	apiKeyID2 := insertAccount("duplicate-apikey-2", service.AccountTypeAPIKey, false)
	deletedID := insertAccount("duplicate-deleted", service.AccountTypeAPIKey, true)
	for _, binding := range []struct {
		accountID int64
		priority  int
	}{{apiKeyID1, 37}, {apiKeyID2, 8}, {deletedID, 3}} {
	...
	s.Require().EqualValues(2, duplicate.AccountCount)
	...
	copiedPriorities := make(map[int64]int)
	for rows.Next() {
		var copiedAccountID int64
		var copiedPriority int
		s.Require().NoError(rows.Scan(&copiedAccountID, &copiedPriority))
		copiedPriorities[copiedAccountID] = copiedPriority
	}
	s.Require().Equal(map[int64]int{apiKeyID1: 37, apiKeyID2: 8}, copiedPriorities,
		"active accounts must be copied with their priorities; soft-deleted accounts must not be copied")
...
func (s *GroupRepoSuite) TestListWithFilters_AccountCount() {
	...
	s.Require().NoError(scanSingleRow(
		...
		[]any{"acc1", service.PlatformAnthropic, service.AccountTypeAPIKey},
		&accountID,
	))
...
func (s *GroupRepoSuite) TestGetAccountCount() {
	...
	s.Require().NoError(scanSingleRow(
		...
		s.tx,
		"INSERT INTO accounts (name, platform, type) VALUES ($1, $2, $3) RETURNING id",
		[]any{"a1", service.PlatformAnthropic, service.AccountTypeAPIKey},
		&a1,
	))
	var a2 int64
	s.Require().NoError(scanSingleRow(
		...
		s.tx,
		"INSERT INTO accounts (name, platform, type) VALUES ($1, $2, $3) RETURNING id",
		[]any{"a2", service.PlatformAnthropic, service.AccountTypeAPIKey},
		&a2,
	))
...
func (s *GroupRepoSuite) TestListWithFilters_ActiveAccountCount_LessThanTotal() {
	...
	insertAccount := func(name, status string, schedulable bool) int64 {
		...
		s.Require().NoError(scanSingleRow(
			...
			[]any{name, service.PlatformAnthropic, service.AccountTypeAPIKey, status, schedulable},
			&id,
		))
		return id
	}
...
func (s *GroupRepoSuite) TestListWithFilters_RateLimitedAccountCount() {
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.tx,
		"INSERT INTO accounts (name, platform, type) VALUES ($1, $2, $3) RETURNING id",
		[]any{"acc-normal", service.PlatformAnthropic, service.AccountTypeAPIKey},
		&normalID))
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.tx,
		...
		[]any{"acc-rate-limited", service.PlatformAnthropic, service.AccountTypeAPIKey},
		&rateLimitedID))
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.tx,
		...
		[]any{"acc-overloaded", service.PlatformAnthropic, service.AccountTypeAPIKey},
		&overloadedID))
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.tx,
		...
		[]any{"acc-temp-unschedulable", service.PlatformAnthropic, service.AccountTypeAPIKey},
		&tempUnschedulableID))
	...
	s.Require().NoError(scanSingleRow(s.ctx, s.tx,
		...
		[]any{"acc-expired", service.PlatformAnthropic, service.AccountTypeAPIKey},
		&expiredID))
...
func (s *GroupRepoSuite) TestDeleteAccountGroupsByGroupID() {
	...
	s.Require().NoError(scanSingleRow(
		...
		s.tx,
		"INSERT INTO accounts (name, platform, type) VALUES ($1, $2, $3) RETURNING id",
		[]any{"acc-del", service.PlatformAnthropic, service.AccountTypeAPIKey},
		&accountID,
	))
...
func (s *GroupRepoSuite) TestDeleteAccountGroupsByGroupID_MultipleAccounts() {
	...
	insertAccount := func(name string) int64 {
		...
		s.Require().NoError(scanSingleRow(
			...
			s.tx,
			"INSERT INTO accounts (name, platform, type) VALUES ($1, $2, $3) RETURNING id",
			[]any{name, service.PlatformAnthropic, service.AccountTypeAPIKey},
			&id,
		))
		return id
	}
