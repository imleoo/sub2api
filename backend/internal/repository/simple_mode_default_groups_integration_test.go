...
func TestEnsureSimpleModeDefaultGroups_CreatesMissingDefaults(t *testing.T) {
	...
	// fork：simple 模式默认组只建 anthropic/openai/gemini 三平台（grok/antigravity 不自动建组，
	// 上游 grok-default 图像回填逻辑仅对运营手工场景生效，见 Backfills 测试）。
	grokExists, err := client.Group.Query().
		Where(group.NameEQ(service.PlatformGrok+"-default"), group.DeletedAtIsNil()).
		Exist(seedCtx)
	require.NoError(t, err)
	require.False(t, grokExists)
}
