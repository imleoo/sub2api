...
func TestGrokContentPolicy403DoesNotMutateOrFailover(t *testing.T) {
	...
	account := &Account{ID: 4715, Platform: PlatformGrok, Type: AccountTypeAPIKey}
...
func TestGrokContentPolicySSEErrorDoesNotMutateOrFailover(t *testing.T) {
	...
	account := &Account{ID: 4721, Platform: PlatformGrok, Type: AccountTypeAPIKey, Concurrency: 1}
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
...
func TestHandleGrokAccountUpstreamErrorEntitlement403KeepsDefaultCooldown(t *testing.T) {
	repo := &grokQuotaAccountRepo{}
	svc := &OpenAIGatewayService{accountRepo: repo}
	account := &Account{ID: 4716, Platform: PlatformGrok, Type: AccountTypeAPIKey}
	before := time.Now()
