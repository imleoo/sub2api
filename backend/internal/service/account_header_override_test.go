...
func TestApplyHeaderOverridesNoOpPaths(t *testing.T) {
	...
	// 不合格平台/类型（gemini apikey）：即使配置了覆写也不生效
	ineligible := headerOverrideTestAccount(PlatformGemini, AccountTypeAPIKey, map[string]any{
		credKeyHeaderOverrideEnabled: true,
		credKeyHeaderOverrides:       map[string]any{"user-agent": "hacked"},
	})
	h := baseline()
	ineligible.ApplyHeaderOverrides(h)
