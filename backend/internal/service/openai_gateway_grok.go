...
func (s *OpenAIGatewayService) forwardGrokResponses(
	...
	// fork：仅支持官方 xAI API（apikey 账号）；grok.com 订阅逆向 OAuth 链已删除（功能 35）。
	if account.Type != AccountTypeAPIKey {
		return nil, fmt.Errorf("grok account type %s is not supported (official xAI API requires api_key)", account.Type)
	}
	...
	patchedBody, err = applyGrokResponsesCacheIdentity(patchedBody, body, cacheIdentity, false /* fork: 无 OAuth 账号，Free-tier 工具注入恒关 */)
	if err != nil {
...
// grokQuotaSnapshotFromExtra 从账号 Extra 反序列化被动配额快照。
func grokQuotaSnapshotFromExtra(extra map[string]any) (*xai.QuotaSnapshot, error) {
	...
	case map[string]any:
		...
		return &out, nil
	default:
		return nil, fmt.Errorf("unexpected grok quota snapshot type %T", raw)
	}
}
