...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_CodexImageBridgeRespectsResponsesLite(t *testing.T) {
	...
	// fork：命名空间摊平（namespace -> input.additional_tools 合并）原本只对
	// Codex OAuth 账号生效；OAuth 逆向订阅链已删（功能 35），
	// shouldFlattenOpenAIResponsesNamespaces() 恒为 false，apikey 账号的
	// namespace 工具不再被摊平合并进 additional_tools，原样保留在 tools 中。
	require.True(t, gjson.Get(litePayload, `tools.#(type=="namespace")`).Exists())
	require.Equal(t, "collaboration", gjson.Get(litePayload, `tools.#(type=="namespace").name`).String())
...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_PassthroughHeadersUsePromptCacheAndTurnState(t *testing.T) {
	...
	// fork：命名空间摊平（namespace -> input.additional_tools）原本只对 Codex
	// OAuth 账号生效；OAuth 逆向订阅链已删（功能 35），
	// shouldFlattenOpenAIResponsesNamespaces() 恒为 false，apikey 账号的
	// passthrough 请求体不再被改写，namespace 工具原样透传给上游。
	require.True(t, gjson.Get(forwarded, `tools.#(type=="namespace")`).Exists())
	require.Equal(t, "collaboration", gjson.Get(forwarded, `tools.#(type=="namespace").name`).String())
	require.Equal(t, "spawn_agent", gjson.Get(forwarded, `tools.#(type=="namespace").tools.0.name`).String())
	require.False(t, gjson.Get(forwarded, `input.#(type=="additional_tools")`).Exists())
