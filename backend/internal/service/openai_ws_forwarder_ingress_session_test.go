...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_CodexImageBridgeRespectsResponsesLite(t *testing.T) {
	...
	// responses-lite 端点的命名空间折叠是 normalizeOpenAIResponsesLiteTools 自身
	// 契约的一部分（namespace 工具必须经 input.additional_tools 承载），与账号鉴权
	// 方式无关，不受 shouldFlattenOpenAIResponsesNamespaces() 的 OAuth-only 门控影响。
