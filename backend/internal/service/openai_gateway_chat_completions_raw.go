...
// grokChatRawEndpoint 官方 xAI chat/completions 端点（原定义随 OAuth chat 桥文件删除后移入本文件，功能 35）。
const grokChatRawEndpoint = "/v1/chat/completions"
...
func (s *OpenAIGatewayService) forwardAsRawChatCompletions(
	...
	targetURL, err := s.rawChatCompletionsURL(ctx, account)
	if err != nil {
		return nil, err
	}
...
func (s *OpenAIGatewayService) rawChatCompletionsURL(ctx context.Context, account *Account) (string, error) {
	// 功能 25：generic 渠道走账号绑定的 openai 协议端点 base_url（凭证由 GetAccessToken 提供账号级 key）。
	if account.IsGeneric() {
		baseURL := s.genericOpenAIBaseURL(ctx, account)
		if baseURL == "" {
			return "", fmt.Errorf("account %d has no schedulable openai endpoint", account.ID)
		}
		validatedURL, err := s.validateUpstreamBaseURL(baseURL)
		if err != nil {
			return "", fmt.Errorf("invalid generic endpoint base_url: %w", err)
		}
