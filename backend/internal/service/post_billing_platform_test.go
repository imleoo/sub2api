...
// TestQuotaPlatform 锁定配额计量口径：ForcePlatform 路由按 ForcePlatform 计，
...
func TestQuotaPlatform(t *testing.T) {
	...
	t.Run("force platform overrides group platform", func(t *testing.T) {
		...
		if got := QuotaPlatform(ctx, apiKey); got != PlatformGemini {
			t.Errorf("QuotaPlatform with force = %q, want %q", got, PlatformGemini)
		}
	})
	...
	t.Run("nil api key with force platform returns force platform", func(t *testing.T) {
		ctx := context.WithValue(context.Background(), ctxkey.ForcePlatform, PlatformGemini)
		if got := QuotaPlatform(ctx, nil); got != PlatformGemini {
			t.Errorf("QuotaPlatform(nil) with force = %q, want %q", got, PlatformGemini)
		}
	})
}
