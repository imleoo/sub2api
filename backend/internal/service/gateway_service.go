...
# Tone and style
 ...
 - Do not use a colon before tool calls. Your tool calls may not be shown directly in the output, so text like "Let me read the file:" followed by a read tool call should just be "Let me read the file." with a period.`
	...
	debugGatewayBodyEnv          = "TOKENPANEL_DEBUG_GATEWAY_BODY"
...
type GatewayService struct {
	...
	debugGatewayBodyFile  atomic.Pointer[os.File] // non-nil when TOKENPANEL_DEBUG_GATEWAY_BODY is set
...
func NewGatewayService(
	...
	svc.debugModelRouting.Store(parseDebugEnvBool(os.Getenv("TOKENPANEL_DEBUG_MODEL_ROUTING")))
	svc.debugClaudeMimic.Store(parseDebugEnvBool(os.Getenv("TOKENPANEL_DEBUG_CLAUDE_MIMIC")))
...
// tokenpanelUserID: 系统用户 ID，用于二维亲和调度
func (s *GatewayService) SelectAccountWithLoadAwareness(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, metadataUserID string, tokenpanelUserID int64) (*AccountSelectionResult, error) {
	// 调试日志：记录调度入口参数
...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (*ForwardResult, error) {
	...
	if account != nil && account.IsResponseMaskingEnabled() && c != nil {
		if lastText := extractLastUserTextFromRaw(parsed.MessagesRaw()); isIdentityQuestion(lastText) {
	...
	for attempt := 1; attempt <= maxRetryAttempts; attempt++ {
		...
		upstreamReq, wireBody, err := s.buildUpstreamRequest(upstreamCtx, c, account, body, token, tokenType, reqModel, reqStream)
		releaseUpstreamCtx()
		if err != nil {
			return nil, err
		}
		...
		if resp.StatusCode == 400 {
			...
			if readErr == nil {
				...
				if s.shouldRectifySignatureError(ctx, account, respBody, reqModel) {
					...
					retryReq, retryWireBody, buildErr := s.buildUpstreamRequest(retryCtx, c, account, filteredBody, token, tokenType, reqModel, reqStream)
					releaseRetryCtx()
					if buildErr == nil {
						...
						if retryErr == nil {
							...
							if retryReadErr == nil && retryResp.StatusCode == 400 && s.isSignatureErrorPattern(ctx, account, retryRespBody) {
								...
								if looksLikeToolSignatureError(msg2) && time.Since(retryStart) < maxRetryElapsed {
									...
									retryReq2, retryWireBody2, buildErr2 := s.buildUpstreamRequest(retryCtx2, c, account, filteredBody2, token, tokenType, reqModel, reqStream)
									releaseRetryCtx2()
									if buildErr2 == nil {
				...
				if isThinkingBudgetConstraintError(errMsg) && s.settingService.IsBudgetRectifierEnabled(ctx) {
					...
					if applied && time.Since(retryStart) < maxRetryElapsed {
						...
						budgetRetryReq, budgetWireBody, buildErr := s.buildUpstreamRequest(budgetRetryCtx, c, account, rectifiedBody, token, tokenType, reqModel, reqStream)
...
func (s *GatewayService) buildUpstreamRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string, reqStream bool) (*http.Request, []byte, error) {
	...
	finalBetaHeader, finalBetaShouldSet := s.computeFinalAnthropicBeta(
		clientHeaders, body, effectiveDropSet,
	)
...
// 计算纯函数。当前与 computeFinalAnthropicBeta 行为一致（仅 API-key 客户端透传 / InjectBeta
// 注入路径）；保留独立函数以便 count_tokens 专属规则将来再次分化时不必改调用方。
...
func (s *GatewayService) ForwardCountTokens(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) error {
	...
	// 构建上游请求
	upstreamReq, wireBody, err := s.buildCountTokensRequest(ctx, c, account, body, token, tokenType, reqModel)
	if err != nil {
	...
	if resp.StatusCode == 400 && s.shouldRectifySignatureError(ctx, account, respBody, reqModel) {
		...
		filteredBody := FilterThinkingBlocksForRetry(body, reqModel)
		retryReq, retryWireBody, buildErr := s.buildCountTokensRequest(ctx, c, account, filteredBody, token, tokenType, reqModel)
		if buildErr == nil {
...
func (s *GatewayService) buildCountTokensRequest(ctx context.Context, c *gin.Context, account *Account, body []byte, token, tokenType, modelID string) (*http.Request, []byte, error) {
	// 确定目标 URL
	targetURL := claudeAPICountTokensURL
	if account.Type == AccountTypeAPIKey {
		baseURL := account.GetBaseURL()
	...
	ctEnableCCH := false
	if s.settingService != nil {
		_, _, ctEnableCCH = s.settingService.GetGatewayForwardingSettings(ctx)
	}
	...
	finalBetaHeader, finalBetaShouldSet := s.computeFinalCountTokensAnthropicBeta(
		clientHeaders, body, ctEffectiveDropSet,
	)
...
// 启用方式（环境变量）：
//
//	TOKENPANEL_DEBUG_GATEWAY_BODY=1                          # 写入 gateway_debug.log
//	TOKENPANEL_DEBUG_GATEWAY_BODY=/tmp/gateway_debug.log     # 写入指定路径
//
