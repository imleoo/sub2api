...
// CalculatePerSecondVideoCost 计算视频生成费用（灵境/万界按秒口径）：cost = output_cost_per_image (USD/秒) × seconds × multiplier × discount_rate。
...
func (s *BillingService) CalculatePerSecondVideoCost(model string, seconds float64, rateMultiplier float64) *CostBreakdown {
	if seconds <= 0 {
		return &CostBreakdown{}
	}
