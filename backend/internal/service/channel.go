...
const (
	...
	BillingModeVideo      BillingMode = "video"       // 视频计费：cost = output_cost_per_image (USD/秒) × seconds
)
...
type ChannelModelPricing struct {
	...
	ImageInputPrice  *float64          // 图片输入 token 价格（如 gpt-image-2 图片编辑 / 多模态 embedding 图文不同价）；未配置时回退文本输入价
