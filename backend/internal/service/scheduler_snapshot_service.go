...
func schedulerSnapshotPlatforms() [5]string {
	return [5]string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformLingjing, PlatformGrok}
}
...
func (s *SchedulerSnapshotService) loadAccountsFromDB(ctx context.Context, bucket SchedulerBucket, useMixed bool) ([]Account, error) {
	...
	// mixed 桶（fork 的跨平台混合调度账号已随逆向平台移除，IsMixedSchedulingEnabled()
	// 恒为 false）仍需走独立的 Platforms 查询，与 single/forced 的缓存 key 区分开，
	// 保持 schedulerAccountQueryCache「mixed 与历史模式保持独立」的既有约定。
	if useMixed {
