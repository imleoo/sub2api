...
// duplicateAccountBaseRepoStub 是 DuplicateAccount 测试用的最小内存账号仓储桩。
// 曾经与 spark 影子账号测试共用一个基础桩（sparkShadowRepoStub），该功能随
// 功能 35 订阅逆向清理整体移除后，这里保留纯通用的 Create/GetByID 部分。
type duplicateAccountBaseRepoStub struct {
...
func newDuplicateAccountBaseRepoStub() *duplicateAccountBaseRepoStub {
	return &duplicateAccountBaseRepoStub{
...
func (s *duplicateAccountBaseRepoStub) Create(_ context.Context, account *Account) error {
	s.nextID++
	account.ID = s.nextID
	cp := *account
...
func (s *duplicateAccountBaseRepoStub) GetByID(_ context.Context, id int64) (*Account, error) {
...
func newDuplicateAccountRepoStub() *duplicateAccountRepoStub {
	return &duplicateAccountRepoStub{
		duplicateAccountBaseRepoStub: newDuplicateAccountBaseRepoStub(),
...
func TestDuplicateAccountRejectsRotatingOrUnknownCredentialTypes(t *testing.T) {
	for _, accountType := range []string{"legacy-cookie", "legacy-setup-token"} {
