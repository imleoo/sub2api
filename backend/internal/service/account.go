...
// IsOpenAICompatible 官方 xAI(Grok) API 与 OpenAI 协议兼容，共用 openai 网关。
func (a *Account) IsOpenAICompatible() bool {
	return a != nil && (a.Platform == PlatformOpenAI || a.Platform == PlatformGrok)
}

// GetGrokBaseURL 官方 xAI API base_url（默认 https://api.x.ai/v1）。
...
func (a *Account) IsOpenAIChatGPTSubscription() bool {
	// fork 无 OAuth 账号（功能 35），订阅池改由 apikey 账号的 plan_type 凭证字段标识
	// （运营方在上游本身是订阅制中转时手动填写）。
	if !a.IsOpenAIApiKey() {
		return false
	}
...
func (a *Account) SupportsOpenAIEndpointCapability(capability OpenAIEndpointCapability) bool {
	...
	case OpenAIEndpointCapabilityAlphaSearch:
		// alpha/search 走 {base_url}/v1/alpha/search（见 openAIAlphaSearchURL），
		// fork 仅支持 API key 账号。上游不支持该端点时由转发层 failover 兜底。
		if a.Type != AccountTypeAPIKey {
			return false
		}
...
func (a *Account) GrokMediaGenerationEligibility() (bool, string) {
	...
	// fork 已移除 Grok OAuth 订阅链与 billing 观测（功能 35），Grok 账号均为
	// 官方 apikey 接入，无 OAuth/billing 快照可判定，故除显式 override 外一律放行。
...
func (a *Account) SupportsOpenAIImageCapability(capability OpenAIImagesCapability) bool {
	...
	case OpenAIImagesCapabilityBasic, OpenAIImagesCapabilityNative:
		// /v1/images 是 OpenAI 专属端点，Grok 等 OpenAI 兼容平台不支持。
		return a.IsOpenAI() && a.Type == AccountTypeAPIKey
	default:
		return true
	}
