...
func (s *adminServiceImpl) GetUserUsageStats(ctx context.Context, userID int64, startTime, endTime time.Time) (*usagestats.AccountUsageStatsResponse, error) {
	if userID <= 0 {
		return nil, infraerrors.BadRequest("INVALID_USER_ID", "invalid user ID")
	}
	if s.usageLogRepo == nil {
		return nil, infraerrors.InternalServer("USAGE_STATS_UNAVAILABLE", "usage statistics service is unavailable")
	}
	if _, err := s.userRepo.GetByID(ctx, userID); err != nil {
		return nil, err
	}
	return s.usageLogRepo.GetUserUsageStats(ctx, userID, startTime, endTime)
}
