...
// fork：已移除「grok 分组默认开启生图」的平台特判，
// 所有平台默认不开启生图，仅显式传入 AllowImageGeneration 时开启。
func TestAdminService_CreateGroup_ImageGenerationDefaultsDisabled(t *testing.T) {
	repo := &groupRepoStubForAdmin{}
	svc := &adminServiceImpl{groupRepo: repo}
...
func TestAdminService_ListGroups_WithSearch(t *testing.T) {
	...
	t.Run("search 与其他过滤条件组合使用", func(t *testing.T) {
		...
		groups, total, err := svc.ListGroups(context.Background(), 3, 50, PlatformGemini, StatusActive, "beta", &isExclusive, "", "")
		require.NoError(t, err)
