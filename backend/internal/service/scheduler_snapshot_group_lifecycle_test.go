...
// expectedGroupLifecycleBuckets 复用生产环境的 canonical bucket 构造函数
// （schedulerCanonicalBuckets），而非在测试中手写平台列表，避免两处平台
// 清单（当前为 Anthropic/Gemini/OpenAI/Lingjing 四个）随生产代码演进而失步。
...
func TestSchedulerGroupLifecycleActiveReopensAndRebuildsAllCurrentBuckets(t *testing.T) {
	...
	accounts.beforeLoad = func() {
		...
		require.Equal(t, 10, tokenCount, "all reopen tokens must be prepared before the first account query")
	}
	svc := newGroupLifecycleTestService(cache, accounts, groups, config.RunModeStandard)
	seen := make(map[batchSeenKey]struct{})
	...
	require.Len(t, cache.tokens(), 10)
	require.Equal(t, 6, accounts.callCount())
	...
	require.Contains(t, bucketStrings(current), SchedulerBucket{GroupID: groupID, Platform: PlatformLingjing, Mode: SchedulerModeForced}.String())
...
func TestSchedulerGroupLifecycleEpochPreventsABA(t *testing.T) {
	...
	require.Len(t, firstActiveTokens, 10)
	...
	require.Len(t, allTokens, 20)
	require.Greater(t, allTokens[10].Epoch, firstActiveTokens[0].Epoch)
