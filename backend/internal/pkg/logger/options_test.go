...
func TestResolveLogFilePath_WithDataDir(t *testing.T) {
	t.Setenv("DATA_DIR", "/tmp/tokenpanel-data")
	got := resolveLogFilePath("")
	want := filepath.Join("/tmp/tokenpanel-data", "logs", "tokenpanel.log")
	if got != want {
...
func TestBuildFileCore_InvalidPathFallback(t *testing.T) {
	...
	opts.Output.FilePath = filepath.Join(os.DevNull, "logs", "tokenpanel.log")
